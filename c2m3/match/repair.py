...
def repair_model(model_to_repair, models, train_loader):
    ...
    if torch.cuda.is_available():
        ...
        repaired_model = make_tracked_net(model_to_repair).cpu()
    ...
    if torch.cuda.is_available():
        ...
        reset_bn_stats(repaired_model.cpu(), loader=train_loader_merged, epochs=1)
    ...
    if isinstance(model_to_repair.model, ResNet):
        ...
        if torch.cuda.is_available():
            ...
            repaired_model_correct_class = RepairedResNet(**kwargs).cpu().eval()
