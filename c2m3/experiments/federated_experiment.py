...
class FederatedExperimentRunner:
    ...
    def __init__(self, config: FederatedExperimentConfig, use_direct_partitioning: bool = True):
        ...
        # Removed federated_partition_dir since we're not using it anymore
        self.client_ids = []
    ...
    def setup(self):
        ...
        # Fix: use num_total_clients instead of num_clients_per_round to create enough partitions for all clients
        train_partitions, val_partitions, test_set = self.data_manager.partition_data(data_refs, self.config.num_total_clients)
        ...
        logger.info(f"Created {len(self.client_ids)} client IDs for simulation")
        ...
        # Remove the federated partition directory creation and client data mappings saving
        # Instead, just initialize the parameters and history
    ...
    def _evaluate_global_model(self, parameters: NDArrays) -> Dict[str, float]:
        ...
        if self.config.dataset_name == "femnist":
            ...
            if hasattr(self, 'test_dataset') and hasattr(self, 'global_test_loader'):
                # TODO write dedicated evaluation function for CIFAR10
                criterion = nn.CrossEntropyLoss()
                total_loss = 0.0
                correct = 0
                total = 0
    ...
    def run(self):
        ...
        # Create client generator based on whether we're using direct dataset objects
        if hasattr(self, 'train_partitions') and self.train_partitions is not None:
            federated_client_generator = get_flower_dataset_client_generator(
                model_generator=self.model_generator,
                train_partitions=self.train_partitions,
                val_partitions=self.val_partitions
            )
            logger.info(f"Using dataset-based client generator with {len(self.train_partitions)} partitions (one per client)")
            logger.info(f"Each client also has a validation partition with {len(self.val_partitions[0])} samples on average")
        else:
            logger.error("No train_partitions available. Cannot run experiment with file-based client generator.")
            raise ValueError("No train_partitions available. To use file-based client generator, you need to implement your own partition saving mechanism.")
        ...
        if self.config.num_evaluation_clients > 0:
            ...
            logger.info("These clients will use their validation partitions for evaluation")
        else:
        ...
        if self.config.dataset_name == "femnist":
            # For FEMNIST, use the new DatasetFrankWolfeSync strategy
            logger.info("Using DatasetFrankWolfeSync strategy for FEMNIST with dataset objects")
            strategy = DatasetFrankWolfeSync(
                train_datasets=self.train_partitions,
            ...
            # For CIFAR10, use the FrankWolfeSyncTinyResNet strategy
            ...
            # For other datasets, use the standard FedAvg
            logger.warning(f"Using standard FedAvg strategy for {self.config.dataset_name} (not recommended)")
            strategy = FedAvg(
                fraction_fit=fraction_fit,
                fraction_evaluate=fraction_evaluate,
                min_fit_clients=self.config.num_clients_per_round,
        ...
        logger.info(f"Number of total clients: {self.config.num_total_clients}")
        logger.info(f"Number of clients per round: {self.config.num_clients_per_round}")
        logger.info(f"Each client has its own unique partition (total partitions: {len(self.train_partitions)})")

        try:
            self.parameters_for_each_round, self.history = start_seeded_simulation(
                client_fn=simulator_client_generator,
                num_clients=self.config.num_total_clients,  # Changed from len(self.client_ids)
    ...
    # def use_direct_data_partitioning(self, enabled=True):
    #     """Enable or disable direct data partitioning using corrected_partition_data.
        
    #     Args:
    #         enabled: True to use direct partitioning, False to use predefined partitions
    #     """
    #     self.use_direct_partitioning = enabled
    #     return self

    # def use_predefined_partitions(self, data_distribution=None):
    #     """Use predefined partitions instead of direct data partitioning.
        
    #     This method is a convenient way to switch to using predefined partitions.
    #     If data_distribution is provided, it will override the config's data_distribution.
        
    #     Args:
    #         data_distribution (str, optional): Override the data distribution type
    #             Options: 'iid', 'dirichlet', 'pathological', 'natural'
                
    #     Returns:
    #         FederatedExperimentRunner: self for method chaining
    #     """
    #     self.use_direct_partitioning = False
        
    #     # Optionally override the data distribution
    #     if data_distribution is not None:
    #         valid_distributions = ['iid', 'dirichlet', 'pathological', 'natural'] 
    #         if data_distribution not in valid_distributions:
    #             logger.warning(f"Invalid data distribution: {data_distribution}. "
    #                            f"Must be one of: {valid_distributions}")
    #         else:
    #             self.config.data_distribution = data_distribution
    #             logger.info(f"Changed data distribution to: {data_distribution}")
...
if __name__ == "__main__":
    # Example configuration for FEMNIST
    femnist_config = FederatedExperimentConfig(
        experiment_name="fed_experiment_lr",
        dataset_name="femnist",
        model_name="cnn",
        num_rounds=10,
        local_epochs=20,
    ...
    cifar10_config = FederatedExperimentConfig(
        experiment_name="cifar10_federated_experiment",
        dataset_name="cifar10",
        model_name="tiny_resnet",  # Use our custom TinyResNet model
        num_rounds=10,
        local_epochs=30,  # Increased from 1 to 30
        learning_rate=0.01,
        server_learning_rate=1.0,
        data_distribution="dirichlet",
        non_iid_alpha=0.5,
        samples_per_partition=6000  # Increased from 4000 to 6000
    )
    
    # Choose which configuration to use
    config_to_use = femnist_config
    
    # Example 1: Using direct partitioning (default)
    # runner = FederatedExperimentRunner(config_to_use)
    # runner.setup()
    # results = runner.run()
    # runner.visualize_results()
    
    # Example 2: Using predefined partitions
    # This will look for data in: data_dir/femnist/client_data_mappings/fed_natural
    # runner = FederatedExperimentRunner(config_to_use, use_direct_partitioning=False)
    # # Or alternatively:
    # runner = FederatedExperimentRunner(config_to_use).use_predefined_partitions(data_distribution="natural")
    # runner.setup()
    # results = runner.run()
    # runner.visualize_results()
    
    # Example 3: Comparing different strategies by modifying the strategy generation
    # To be implemented in a future update
    
    # Example parameter sweeps (uncomment to run)
    # run_learning_rate_experiment(config_to_use, [0.001, 0.01, 0.1])
    # run_server_lr_experiment(config_to_use, [0.1, 0.5, 1.0, 1.5])
    # run_local_epochs_experiment(config_to_use, [1, 5, 10, 15])
    # run_multi_seed_experiment(config_to_use, [42, 123, 456, 789, 101])
